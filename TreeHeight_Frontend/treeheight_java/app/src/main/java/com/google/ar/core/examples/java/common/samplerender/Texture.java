...
public class Texture implements Closeable {
   ...
   */
  public enum Target {
    TEXTURE_2D(GLES30.GL_TEXTURE_2D), //Target.TEXTURE=GLES30.GL_TEXTURE_2D
    TEXTURE_EXTERNAL_OES(GLES11Ext.GL_TEXTURE_EXTERNAL_OES),//实现相机预览的每一帧的输出格式是YUV的（YUV），这个扩展纹理的作用就是实现YUV格式到RGB的自动转化，我们就不需要再为此写YUV转RGB的代码了
