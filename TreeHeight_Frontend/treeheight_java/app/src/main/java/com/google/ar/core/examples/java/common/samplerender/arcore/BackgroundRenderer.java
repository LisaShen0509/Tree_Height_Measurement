...
public class BackgroundRenderer {
  ...
  public BackgroundRenderer(SampleRender render) {
    cameraColorTexture =
        new Texture(
            render,
            Texture.Target.TEXTURE_EXTERNAL_OES,//yuv to rgb纹理
            Texture.WrapMode.CLAMP_TO_EDGE,
            /*useMipmaps=*/ false);
    cameraDepthTexture =
        new Texture(
            render,
            Texture.Target.TEXTURE_2D,//2d纹理
            Texture.WrapMode.CLAMP_TO_EDGE,
            /*useMipmaps=*/ false);

//    创建具有三个顶点缓冲区的网格：一个用于屏幕坐标（归一化设备坐标），
//    一个用于相机纹理坐标（稍后在绘制前使用适当的数据填充），
//    另一个用于虚拟场景纹理坐标（单位纹理四边形）
  ...
  public void setUseDepthVisualization(SampleRender render, boolean useDepthVisualization)
    ...
    if (useDepthVisualization) {
     depthColorPaletteTexture =  //Texture类型
